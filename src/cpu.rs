...
use crate::display::Sprite as Sprite;
...
fn or( &mut self, reg1: u16, reg2: u16) {
    ...
    self.mem.v_regs[reg1 as usize] = self.mem.v_regs[reg1 as usize] | self.mem.v_regs[reg2 as usize];
    self.pc +=2;
}
...
fn addr( &mut self, reg1: u16, reg2: u16) {
    ...
    self.mem.v_regs[reg1 as usize] = ((self.mem.v_regs[reg1 as usize] as i32 + self.mem.v_regs[reg2 as usize] as i32)%256) as u8;
    self.pc +=2;
...
fn shr( &mut self, reg: u16) {
    ...
    self.mem.v_regs[reg as usize] = self.mem.v_regs[reg as usize]  >> 1;
    self.pc +=2;
}
fn subn( &mut self, reg1: u16, reg2: u16) {
    ...
    self.mem.v_regs[reg1 as usize] = ((self.mem.v_regs[reg2 as usize] as i32 - self.mem.v_regs[reg1 as usize] as i32)%255) as u8;
    self.pc +=2;
...
fn skp( &mut self, reg: u16) {
    ...
    if self.input.keys[self.mem.v_regs[reg as usize] as usize] == true
    {
        self.pc +=2;
    }
    self.pc +=2;
...
fn sknp( &mut self, reg: u16) {
    println!("skip if key {:x} NOT pressed",self.mem.v_regs[reg as usize]);
    //DEBUG: DUMP KEYSTATES
    self.input.dump();
...
fn addireg( &mut self, reg: u16) {
    ...
    self.mem.I = self.mem.I + self.mem.v_regs[reg as usize] as u16;
    self.pc +=2;
}
